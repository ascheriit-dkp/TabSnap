...
assert(manifest.minimum_chrome_version === undefined, 'Firefox manifest must not contain minimum_chrome_version.');
assert(sameStrings(manifest.permissions, ['tabs', 'tabGroups']), 'Firefox permissions must stay tabs + tabGroups.');
assert(
  sameStrings(manifest.optional_host_permissions, ['http://127.0.0.1/*']),
  'Loopback companion access must remain optional.',
);

assert(manifest.background?.service_worker === undefined, 'Firefox must not depend on background.service_worker.');
assert(
...
assert(typeof gecko?.id === 'string' && gecko.id.length > 0, 'Firefox MV3 signing needs a Gecko extension ID.');
assert(gecko.strict_min_version === '139.0', 'Firefox support baseline must remain 139.0 for full tabGroups support.');
assert(
...
assert(manifest.host_permissions === undefined, 'Firefox spike must not introduce required host permissions.');
